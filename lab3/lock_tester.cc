...
test5(void *x)
{
  ...
  for (int j = 0; j < 10; j++) {
    tprintf("test5: client [%3d] [%3d] in loop....\n",i,j);

    if (i < 5)  lc[0]->acquire(a);
    else  lc[1]->acquire(a);

    tprintf("test5: client [%3d] acquired for lock\n",i);

    check_grant(a);
    tprintf ("test5: client [%3d] got lock\n", i);
    check_release(a);
    if (i < 5) lc[0]->release(a);
    else lc[1]->release(a);

    tprintf("test5: client [%3d] released lock\n",i);
  }
  tprintf("test5 on client [%3d] ended\n",i);
  return 0;
}
