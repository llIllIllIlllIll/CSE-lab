...
getattr(yfs_client::inum inum, struct stat &st)
{
	...
	st.st_mtime = info.mtime;
	st.st_ctime = info.ctime;
	printf("ZISHUBUDING # 5374 BUDABIE BB!\n");
    }
...
{
    printf("*********FUSESERVER:CREATE [%s]FILETYPE [%d]*********\n",name,type);
    int ret;
...
//================================================================================================
void softpudding_symlink(fuse_req_t req,const char * link, fuse_ino_t parent, const char * name){
	printf("SOFTPUDDING_SYMLINK:LINK[%s]PARENT[%d]NAME[%s]\n",link,parent,name);
	yfs_client::inum ino;
	yfs_client::status ret;
	
	
	ret=yfs->softpudding_symlink(link,parent,name,ino);
	if(ret != yfs_client::OK){
		fuse_reply_err(req,EEXIST);
	}
	...
	fep.generation = 0;
	getattr(ino,fep.attr);

	fuse_reply_entry(req,&fep);
	printf("SOFTPUDDING_SYMLINK FINISHED (INO:[%d])\n",ino);	
}
void softpudding_readlink(fuse_req_t req,fuse_ino_t ino){
	printf("SOFTPUDDING_READLINK:READ[%d]\n",ino);
	std::string content;
	yfs->softpudding_readlink(ino,content);
...
main(int argc, char *argv[])
{
     ...
     * rmdir, etc.
     * */
	fuseserver_oper.symlink = softpudding_symlink;
	fuseserver_oper.readlink = softpudding_readlink;
    printf("123 start\n");
